...
static int tryexit(int lookx, int looky, int ienm, int loccom, int irun) {
	...
	if (iqx < 1 || iqx > 8 || iqy < 1 || iqy > 8 ||
		...
		return 0; /* no can do -- neg energy, supernovae, or >8 클링온s */
  ...
  */
	...
	if (nextx != comx || nexty != comy) {
		...
		if (damage[DSRSENS] == 0 || condit == IHDOCKED) {
			proutn("***");
			cramen(ienm);
			if (kdist[loccom] < dist1) proutn(" 이(가) 전진합니다:");
			else proutn(" 이(가) 후퇴합니다:");
...
void movcom(void) {
	...
	/* Figure out which 클링온 is the commander (or Supercommander)
	   and do move */
	...
	/* if skill level is high, move other 클링온s and Romulans too!
	...
	for (i = 1; i <= inplan; i++) {
		if (d.plnets[i].x==d.isx && d.plnets[i].y==d.isy &&
			...
			if (REPORTS) {
				...
				*ipage = 1;
				prout("우후라 중위-  \"함장님, 스타플릿 정보부의 보고입니다.");
				proutn("   슈퍼사령관에 의해");
...
void scom(int *ipage) {
	...
	if (iscate==0 && flag) {
		...
		/* look for nearest base without a commander, no Enterprise, and
		   without too many 클링온s, and not already under attack. */
		ifindit = iwhichb = 0;
	...
	if (d.rembase == 0) {
		...
		if (ibqx==d.isx && ibqy == d.isy && d.isx != batx && d.isy != baty) {
			...
			if (*ipage == 0)  pause_game(1);
			*ipage=1;
			proutn("우후라 중위-  \"함장님, 보고드립니다.");
			cramlc(1, d.isx, d.isy);
			skip(1);
			prout("   구역의 스타베이스가 클링온 슈퍼사령관의 공격을 받고 있습니다.");
			proutn("   스타데이트 ");
			cramf(future[FSCDBAS], 0, 1);
			prout(" 까지 버틸 수 있습니다.\"");
			if (resting==0) return;
			prout("스팍-  \"함장님, 휴식은 여기까지 할까요?\"");
			if (ja()==0) return;
			resting = 0;
