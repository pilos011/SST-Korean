...
void events(void) {
	...
	for (;;) {
		...
		if (stdamtim != 1e30 && REPORTS) {
			stdamtim = 1e30;
			prout("우후라 중위- \"함장님, 서브스페이스 무선이 복구되어");
			prout("   감시 보고가 수신되고 있습니다.");
			skip(1);
			for (i=1; i <= 8 ; i++)
				for (j=1; j <= 8; j++)
		...
		switch (line) {
			...
			case FTBEAM: /* Tractor beam */
				...
				if (iscraft==0) {
					skip(1);
					if (Rand() >0.5) {
						prout("갈릴레오가 행성 표면에서 외계인에게 포획되었습니다.");
						prout("플라잉 맥도날드로 개조되었습니다.");
						damage[DSHUTTL] = -10;
						iscraft = -1;
					}
					else {
						prout("갈릴레오가 행성 표면에 잘 숨겨져 있습니다.");
					}
				}
				if (line==0) {
					quadx = d.isx;
			...
			case FBATTAK: /* Commander attacks starbase */
				...
				cramlc(1, batx, baty);
				skip(1);
				prout("이(가) 공격을 받고 있으며 ");
				cramf(future[FCDBAS],1,1);
				proutn(" 스타데이트 까지만 버틸 수 있다고 합니다. ");
				prout("\"");
				if (resting) {
					skip(1);
					proutn("스팍-  \"함장님, 휴식 기간을 취소할까요?\"");
					if (ja()) {
						resting = 0;
						Time = 0.0;
						return;
			...
			case FCDBAS: /* Commander succeeds in destroying base */
				...
				if (batx==quadx && baty==quady) {
					...
					newcnd();
					skip(1);
					prout("스팍-  \"함장님, 스타베이스가 파괴된 것 같습니다.\"");
				}
					...
					ipage = 1;
					skip(1);
					prout("우후라 중위-  \"함장님, 스타플릿 사령부에서 보고했습니다.");
					proutn("   스타베이스가");
					cramlc(1, batx, baty);
					prout(" 에 의해 파괴되었습니다");
					if (isatb==2) prout("클링온 슈퍼사령관");
					else prout("클링온 사령관");
				}
				...
				if (probecx != i || probecy != j) {
					...
					if (i < 1 || i > 8 || j < 1 || j > 8 ||
						...
						if (REPORTS) {
							...
							ipage = 1;
							skip(1);
							proutn("우후라 중위- \"심우주 탐침이 ");
							if (i < 1 ||i > 8 || j < 1 || j > 8)
								proutn("은하를 벗어났습니다");
							else
								proutn("더 이상 신호를 보내지 않습니다");
							prout(".\"");
						}
						future[FDSPROB] = 1e30;
						break;
...
void snova(int insx, int insy) {
	...
	if (insy== 0) {
		...
		if (nqx != quadx || nqy != quady || justin != 0) {
			/* it isn't here, or we just entered (treat as inroute) */
			if (REPORTS) {
				skip(1);
				proutn("스타플릿 사령부로부터 메시지       스타데이트 ");
				cramf(d.date, 0, 1);
				skip(1);
				proutn("     초신성 발생:");
				cramlc(1, nqx, nqy);
	...
	if (d.remkl==0 && (nqx != quadx || nqy != quady)) {
		skip(2);
		if (insx == 0) prout("운이 좋으시군요!");
		proutn("초신성 발생 구역:");
		cramlc(1, nqx, nqy);
		prout(" 에서 마지막 클링온들이 파괴되었습니다.");
		finish(FWON);
		return;
	}
